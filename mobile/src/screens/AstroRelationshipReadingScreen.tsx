...
function validateDrafts(drafts: SubjectDraft[], mode: 'compatibility' | 'family') {
  ...
  if (missing) return 'Her kişi için isim ve doğum tarihi gerekli. Saat ve doğum yeri bilinmiyorsa boş bırakabilirsin.';
...
export function AstroRelationshipReadingScreen({ route, navigation }: Props) {
  ...
  const readyToInterpret = useMemo(() => {
    ...
    return drafts.every((draft) => {
      ...
      return completedManualIds.includes(draft.localId) && draft.displayName.trim() && draft.birthDate.trim();
    });
