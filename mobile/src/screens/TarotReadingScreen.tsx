...
function readingTheme(spreadTitle: string, cards: DrawnTarotCard[]) {
  return `tarot ${spreadTitle}: ${cards
    .slice(0, 5)
...
export function TarotReadingScreen({ route, navigation }: Props) {
  const { profileId, assistantId, spreadId } = route.params;
  ...
  }, [addUsage, assistantId, assistantLabel, cards, profileId, spread, usedClosings]);
  ...
  const persistReadingAndEnd = useCallback(async () => {
    ...
    await appendReadingDerivedTheme(profileId, readingTheme(spread.title, cards), `personal-tarot-${spread.id}-${Date.now()}`).catch(() => {});
    void loadAccountState()
      ...
      .catch(() => {});
    navigation.goBack();
  }, [assistantId, cards, messages, navigation, profileId, profileName, readingText, spread]);
      ...
      <SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
        ...
        >
          ...
          <TouchableOpacity style={[styles.panel, styles.spreadPanel]} activeOpacity={0.9} onPress={() => setSpreadModalVisible(true)}>
            <Text style={styles.spreadHint}>Açılımı büyütmek için dokun</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} scrollEnabled={false}>
              <View style={[styles.spreadCanvas, { width: spreadCanvasWidth, height: canvasHeight }]}>
                {cards.map((card) => {
                  ...
                  const imageSource = TAROT_FRONT_IMAGE_MAP[card.cardName];
                  return (
                    <View
                      key={card.id}
                      style={[
                ...
                <View style={[styles.spreadCanvas, { width: modalCanvasWidth, height: modalCanvasHeight }]}>
                  {cards.map((card) => {
                    ...
                    return (
                      ...
                      >
                        <Image
                          source={TAROT_FRONT_IMAGE_MAP[card.cardName]}
                          style={[styles.cardImage, card.orientation === 'reversed' ? styles.reversedImage : null]}
                          resizeMode="cover"
                        />
                        <Image source={BORDER_IMAGE} style={styles.borderOverlay} resizeMode="stretch" />
          ...
          <View style={styles.cardModalOverlay}>
            <View style={styles.cardModal}>
              {selectedCard ? (
                <>
                  <Image
                    source={TAROT_FRONT_IMAGE_MAP[selectedCard.cardName]}
